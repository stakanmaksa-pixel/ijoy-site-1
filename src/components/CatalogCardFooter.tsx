...
export function CatalogCardFooter({ priceLabel, variantId, canBuy, inStock, availabilityLabel }: {
  ...
  return <div className="mt-auto pt-3">
    <div className="flex min-h-8 flex-col items-center justify-center gap-1 text-center">
      ...
      {availabilityLabel ? <span className="text-xs font-medium text-brand">{availabilityLabel}</span> : !inStock && priceLabel !== PRICE_ON_REQUEST && <span className="text-xs text-zinc-400">{PRICE_ON_REQUEST}</span>}
    </div>

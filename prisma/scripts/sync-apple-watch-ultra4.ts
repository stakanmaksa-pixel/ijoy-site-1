...
async function main() {
  for (const photo of sources.entries) {
    if (!/^apple-watches\/[a-z0-9-]+\.png$/.test(photo.file)) throw Error(`Invalid photo: ${photo.file}`);
  ...
  await prisma.$transaction(async tx => {
    ...
    console.log(`${old ? "EXISTS" : "CREATE"} ${APPLE_WATCH_ULTRA4.name}; новых вариантов: ${plan.variantsToCreate.length}; существующие цены и наличие сохраняются.`);
    ...
    if (!old) {
      ...
      await tx.product.update({ where: { id: old.id }, data: {
        ...photos,
        // Fill absent content; retain edits and all existing offers, even if hidden.
    ...
    console.log("Готово. Ultra 4: Black Titanium / Natural Titanium с Ocean Band. Новые варианты — без цены и подтверждённого наличия.");
